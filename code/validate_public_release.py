#!/usr/bin/env python3
"""Validate the v6.8 public-minimised release boundary, schemas and checksums."""
...
skip_text_scan={
    ...
    'metadata/schema_public_wide_v6_8.json',
    'metadata/schema_public_long_v6_8.json',
    'metadata/schema_public_dictionary_v6_8.json'
}
...
wide_schema=load_json('metadata/schema_public_wide_v6_8.json')
long_schema=load_json('metadata/schema_public_long_v6_8.json')
dict_schema=load_json('metadata/schema_public_dictionary_v6_8.json')
try:
...
if wide is not None and wide_schema:
    if list(wide.columns)!=wide_schema['required_columns']:
        errors.append('Wide table columns do not match schema_public_wide_v6_8.json')
...
if long is not None and long_schema:
    if list(long.columns)!=long_schema['required_columns']:
        errors.append('Long table columns do not match schema_public_long_v6_8.json')
...
if dd is not None and dict_schema and wide is not None:
    if list(dd.columns)!=dict_schema['required_columns']:
        errors.append('Data dictionary columns do not match schema_public_dictionary_v6_8.json')
...
if errors:
    ...
    print('\n'.join(errors))
    sys.exit(1)
print('PASS: v6.8 public-release integrity, schema and boundary audit')
